...
%                                                       %
% Campbell and stability map, hydrodynamics bearings    %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% CONTENT
% In this file, using the bearing parameters, and the previous model for
% system II (with both disks and bearings), Campbell's diagram is
% calculated again with the new bearing configuration (with damping) and
% the stability map of the system is plotted as well.
...
N_campbell=500;      % Number of points in the Campbell's diagram
...
%% MODEL %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 RedFactorId = 0.9; % Inertia reduciton factor
...
Kglob = [   -G*Omega+Damp         K         ; 
 ...
 nnn=192;
     ...
     lambda_d(p(1:nnn))/(2*pi);           % nothing changes?
     %REAL_L((iii-1)*nnn+1:iii*nnn) = real(diag(lambda(p(1:nnn),p(1:nnn)) ))/2/pi;
     %IMAG_L((iii-1)*nnn+1:iii*nnn) = imag(diag(lambda(p(1:nnn),p(1:nnn)) ))/2/pi;
...
findLambda % Plotting the values to find where lambda changes sign
