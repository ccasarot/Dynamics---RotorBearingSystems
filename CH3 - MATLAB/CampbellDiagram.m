...
%                                                       %
%              Copenhagen, April 25th, 2018             %
%                                                       %
%                        Ilmar Ferreira Santos          %
%                                                       %
% OVERHUNG ROTOR   - MODELING AND ANALYSIS              %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% 1 of the report. 
% 
% This file studies the different critical speeds of the three systems:
% Shaft only, one disk and two disks. It does this by plotting the
% Campbell's diagram and using K=10^9 as stiffness for the bearings and a
% small value of Damping to avoid numerical artefacts on the plot

 clear all
 close all

% LOOP for the 3 plots overlap
for indice=1:3
%% General parameters 
...
% Unbalance
m=0;					% Unbalance mass		        [kg]
epsilon=0;				% Radial placement of unbalance	[m]

% Colors for graph
colorSpeed = 'r';
% Orange 
color1 = [230, 159, 0] / 255;      legendText1 = 'Shaft only';
% Green   
color2 = [0, 158, 115] / 255;      legendText3 = 'Shaft + one disk';
% Dark Blue  
color3 = [0, 114, 178] / 255;      legendText2 = 'Shaft + both disks';
...
D_damping = 0.0001; % Overwriting of damping
             
% You need to arrive here with:

% m               % unbalanc emass
% epsilon         % Radial distance of the mass

% Matrices: M, G, K, D_damping
...
figure(1)
plot(Omegarpm,Omegarpm,colorSpeed,'LineWidth',1, 'Color', [1 0 0 1])
grid on
hold on
plot(Omegarpm, lambda_campbells(1:N_campbell, 1), 'Color', color, 'LineWidth', 1.5);
hold on
plot(Omegarpm,lambda_campbells(1:N_campbell,3), 'Color', color, 'LineWidth', 1.5);
hold on
plot(Omegarpm,lambda_campbells(1:N_campbell,5), 'Color', color, 'LineWidth', 1.5);
hold on
plot(Omegarpm,lambda_campbells(1:N_campbell,7), 'Color', color, 'LineWidth', 1.5);
%legend(legendText)
hold on
title('Campbell´s Diagram with Critical Speeds','FontSize',16)
xlabel('Angular Velocity (\Omega) [Hz]','FontSize',14)
ylabel('Natural Frequency (\lambda_i) [Hz]','FontSize',14)
axis([0,AxisScale,0,AxisScale])
