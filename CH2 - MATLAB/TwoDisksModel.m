...
%                                                       %
% Adding both disks to the improved model               %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% 
% Here both of the compressor disks are added to the model.
% Again several versions of the file were created, with different grades of
% precision in the modelling of the disks based on the technical drawings.
% Finally the best solution, giving results close to the experimental data,
% was found for a very detailed discretization of the disks, as described 
% in the report. Also, the disks inertia was increased by 10% to better fit
% the experimental data, creating a more accurate model.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 RAco = 7850;    % {steel density [kg/m^3]}
...
% ------------------ DISK 1
 ...
 Ip_1   = 1/2*(Rd_1^2)*(pi*Rd_1^2*espD_1*RAco) ...
     - 1/2*(Ri_1^2)*(pi*Ri_1^2*espD_1*RAco) ...
     + 1/2*(Rd_1^2)*(pi*Rd_1^2*Lr_1*RAco) ...
     - 1/2*(Rr_1^2)*(pi*Rr_1^2*Lr_1*RAco); % - pi*((Rd_1)^2*Lr_1-(Rr_1)^2*Lr_1)*RAco*Rr_1^2                  
...
% Adding the mass matrices of the disc elements 
   ...
   M((CD1-1)*4+3,(CD1-1)*4+3)=M((CD1-1)*4+3,(CD1-1)*4+3)+Id_1*InertiaFactor;
   M((CD1-1)*4+4,(CD1-1)*4+4)=M((CD1-1)*4+4,(CD1-1)*4+4)+Id_1*InertiaFactor;
